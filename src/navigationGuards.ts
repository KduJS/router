...
import { ComponentOptions, onUnmounted, onActivated, onDeactivated } from 'kdu'
import { inject, getCurrentInstance } from 'kdu'
...
export function onBeforeRouteLeave(leaveGuard: NavigationGuard) {
  ...
  if (!activeRecord) {
    __DEV__ &&
      warn(
        'No active route record was found when calling `onBeforeRouteLeave()`. Make sure you call this function inside of a component child of <router-view>. Maybe you called it inside of App.kdu?'
      )
    return
  }
...
export function onBeforeRouteUpdate(updateGuard: NavigationGuard) {
  ...
  if (!activeRecord) {
    __DEV__ &&
      warn(
        'No active route record was found when calling `onBeforeRouteUpdate()`. Make sure you call this function inside of a component child of <router-view>. Maybe you called it inside of App.kdu?'
      )
    return
  }
    ...
    for (const name in record.components) {
      ...
      if (isRouteComponent(rawComponent)) {
        ...
        const options: ComponentOptions =
          (rawComponent as any).__kccOpts || rawComponent
        ...
        guards.push(() =>
          componentPromise.then(resolved => {
            ...
            // __kccOpts is added by kdu-class-component and contain the regular options
            const options: ComponentOptions =
              (resolvedComponent as any).__kccOpts || resolvedComponent
            const guard = options[guardType]
